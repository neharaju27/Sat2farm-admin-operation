...
export default function NDTI({ onClose, onBack, farmId, clientId }) {
  const [ndtiData, setNDTIData] = useState([]);
  ...
  const fetchNDTIData = async () => {
    ...
    try {
      ...
      console.log('NDTI API Response:', response.data);
      console.log('NDTI Sample Data:', data[0]);
      setNDTIData(data);
      ...
      console.error('NDTI error:', err);
      setError('Failed to fetch NDTI data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchGraphData = async (data) => {
    ...
    const statsEntries = data.filter(
      item => item.zonalStats?.ndti && item.zonalStats.ndti !== 'None'
    );

    // Fetch all stats in parallel for speed
    const results = await Promise.allSettled(
      statsEntries.map(item =>
        axios.get(item.zonalStats.ndti).then(res => ({
          timestamp: item.Date,
          stats: res.data
        }))
      )
    ...
    for (const result of results) {
      if (result.status === 'fulfilled') {
        ...
        console.log('Stats JSON sample:', stats);

        // Try different possible field names for mean NDTI value
        const meanValue =
          stats?.mean ??
          stats?.ndti_mean ??
          stats?.NDTI_mean ??
  ...
  const validImageEntries = ndtiData.filter(item => {
    return (
      item.png?.ndti &&
      item.png.ndti !== 'None'
    );
  ...
  const getChartOption = () => {
    ...
    return {
      ...
      tooltip: {
        trigger: 'axis',
        formatter: (params) => `${params[0].name}<br/>NDTI: ${params[0].value}`
      },
      xAxis: {
        type: 'category',
        ...
        <div className="modal-body" style={{ overflowY: 'auto', padding: '24px' }}>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '60px' }}>
              <div style={{ color: 'var(--text-2)' }}>Loading NDTI data...</div>
            </div>
          )}
          ...
          {/* No data */}
          {!loading && !error && ndtiData.length === 0 && (
            <div style={{
              ...
              <div>
                ...
                <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>
                  Next NDTI data will be available by {getNextAvailableDate()}
                </p>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  {selectedEntry?.png?.ndti && selectedEntry.png.ndti !== 'None' ? (
                    <img
                      src={selectedEntry.png.ndti}
                      alt="NDTI Heatmap"
                      style={{
                        maxWidth: '100%',
                        maxHeight: '280px',
        ...
        >
          ...
          >
            ...
            <p style={{
              ...
              NDTI aims to predict the degree to which the water loses its transparency due to suspended particulates which helps in monitoring the variation of water quality throughout the season respective to the sediments such as debris and other particles present in the water bodies. NDTI data is subjected to available satellite passes and cloud free environment.
            </p>
            <div style={{
              display: 'flex',
              justifyContent: 'flex-end',
