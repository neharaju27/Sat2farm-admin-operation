...
export default function LSWI({ onClose, onBack, farmId, clientId }) {
  ...
  const fetchSatelliteData = async () => {
    ...
    try {
      ...
      const validData = sortedData.filter(item => 
        item.zonalStats && item.zonalStats.lswi && item.zonalStats.lswi !== 'None' &&
        item.png && item.png.lswi && item.png.lswi !== 'None'
      );
  ...
  const fetchChartData = async (data) => {
    ...
    for (const item of data) {
      if (item.zonalStats && item.zonalStats.lswi && item.zonalStats.lswi !== 'None') {
        try {
          const response = await axios.get(item.zonalStats.lswi);
  ...
  const chartOption = chartData && chartData.length > 0 ? {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        const item = params[0];
        return `Date: ${item.name}<br/>Mean Value of LSWI: ${item.value.toFixed(4)}`;
      }
    },
    grid: {
      left: '8%',
            ...
            <>
              <div style={{marginBottom: '20px'}}>
                ...
                {nextReportDate && (
                  <div style={{marginTop: '8px', fontSize: '13px', color: '#64748b'}}>
                    Next LSWI report will be available on: <span style={{fontWeight: '600', color: '#1e293b'}}>{nextReportDate}</span>
                  </div>
                )}
              </div>
              ...
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px'}}>
                <div style={{
                  ...
                  {selectedDate && selectedDate.png && selectedDate.png.lswi && selectedDate.png.lswi !== 'None' ? (
                    <img 
                      src={selectedDate.png.lswi} 
                      alt="LSWI" 
                      style={{
                        width: '100%',
                        height: '300px',
            ...
            <div className="modal-body">
              <div style={{padding: '20px'}}>
                ...
                <p style={{fontSize: '14px', color: 'var(--text-1)', lineHeight: '1.6'}}>
                  LSWI is Land Surface Wetness Index and captures the moisture in the canopy when land is not barren. If the soil is dry, its impact may be seen on the LSWI after a couple of weeks. This should not be used for irrigation scheduling. LSWI data is subjected to available satellite passes and cloud free environment.
                </p>
              </div>
            </div>
            <div className="modal-actions">
