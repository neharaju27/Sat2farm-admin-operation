...
export default function ManagerMonthlyReport({ user, onPageChange }) {
  ...
  const getLoggedInMobileNo = () => {
    ...
    if (storedAuth) {
      try {
        ...
        const mobile = authData.mobile_no || authData.phone_number || authData.phoneNumber;
        if (mobile) return mobile;
      } catch (e) {
        console.error('Error parsing auth data:', e);
      }
    }
    return user?.phone_number || user?.phoneNumber || user?.pNumber;
  };
  const loggedInMobile = getLoggedInMobileNo();

  // Robust month label to query parameter converter (e.g., 'Jan 26' -> 'jan_2026')
  const getMonthQuery = (monthLabel) => {
    if (!monthLabel) return '';
    const cleanLabel = monthLabel.trim();
    
    // 1. Dynamic API mapping
    if (monthQueries && monthQueries[cleanLabel]) {
      return monthQueries[cleanLabel];
    }
    
    // 2. Static lookup
    const staticMap = {
      'Jan 26': 'jan_2026', 'Feb 26': 'feb_2026', 'Mar 26': 'mar_2026', 'Apr 26': 'apr_2026',
      'May 26': 'may_2026', 'Jun 26': 'jun_2026', 'Jul 26': 'jul_2026', 'Aug 26': 'aug_2026',
      'Sep 26': 'sep_2026', 'Oct 26': 'oct_2026', 'Nov 26': 'nov_2026', 'Dec 26': 'dec_2026',
      'Oct 25': 'oct_2025', 'Nov 25': 'nov_2025', 'Dec 25': 'dec_2025'
    };
    if (staticMap[cleanLabel]) {
      return staticMap[cleanLabel];
    }
    
    // 3. Regex parser
    const match = cleanLabel.match(/^([A-Za-z]+)\s+(\d+)$/);
    if (match) {
      const monthPart = match[1].toLowerCase();
      const yearPart = match[2];
      const fullYear = yearPart.length === 2 ? `20${yearPart}` : yearPart;
      return `${monthPart}_${fullYear}`;
    }
    
    return cleanLabel.toLowerCase().replace(/\s+/g, '_');
  };

  const handleDownloadSixMonthReport = async () => {
    if (!loggedInMobile) {
      toast.error('Logged-in user mobile number not found.');
      return;
    }

    const url = `${import.meta.env.VITE_BUSINESS_REPORT_SIX_MONTH_API_URL}?mobile_no=${encodeURIComponent(loggedInMobile)}`;
    const loadingToastId = toast.loading('Fetching 6-month business report...');
    ...
    try {
      ...
      try {
        ...
        // If it successfully parses as JSON, treat it as a backend message instead of a file download
        ...
        // Not JSON - it's a file (CSV/blob). Trigger the file download
        const blob = new Blob([text], { type: contentType || 'text/csv' });
        const blobUrl = window.URL.createObjectURL(blob);
        
        let filename = `business_report_six_month_${loggedInMobile}.csv`;
  ...
  useEffect(() => {
    const fetchAvailableMonths = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data)) {
            ...
            if (months.length > 0) {
              ...
              // Direct immediate fetch to prevent race conditions on initial page load
              fetchMetricsForMonth(mostRecentMonth, queriesMapping[mostRecentMonth]);
            }
          }
        }
      } catch (error) {
        ...
        setSelectedMonth(mostRecentMonth);
        fetchMetricsForMonth(mostRecentMonth, 'mar_2026');
      } finally {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      // Fetch from VITE_BUSINESS_REPORT_API_URL
      const apiUrl = `${import.meta.env.VITE_BUSINESS_REPORT_API_URL}?month=${encodeURIComponent(monthQuery)}&mobile_no=${encodeURIComponent(mobileNo)}`;
      ...
      if (jsonData) {
        if (jsonData.status === 'success' && jsonData.download_link) {
          // Trigger download of the download_link
          const downloadUrl = jsonData.download_link;
          ...
          // Treat as a backend error message (even if status is 404 or 500)
          const errorMsg = jsonData.message || jsonData.error || `Server returned status: ${response.status}`;
          throw new Error(errorMsg);
        }
      } else {
        // Not JSON - check response.ok
        if (!response.ok) {
          throw new Error(`Server returned status: ${response.status}`);
        }
        
        // If ok, download as blob
        const blob = new Blob([text], { type: response.headers.get('content-type') || 'text/csv' });
        const blobUrl = window.URL.createObjectURL(blob);
        const filename = `manager_monthly_report_${selectedMonth.replace(/\s+/g, '_')}.csv`;
        const link = document.createElement('a');
        link.href = blobUrl;
        link.download = filename;
        document.body.appendChild(link);
      ...
      setReportError(`Failed to export CSV: ${error.message}`);
        ...
        <div className="content-area">
          ...
          {!loadingMonths && !loadingMetrics && !reportError && (
            <>
              ...
              <div className="monthly-report-tabs-wrapper">
                <div className="monthly-report-chips">
                ...
                <button
                  onClick={handleDownloadSixMonthReport}
                  className="month-chip active monthly-report-download-btn"
                >
                  <Download className="w-3.5 h-3.5" style={{ display: 'inline-block' }} />
                  Download Last 6 Month Report
                </button>
              </div>
