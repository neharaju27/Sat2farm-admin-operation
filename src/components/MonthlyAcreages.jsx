...
export default function MonthlyAcreages({ user, onPageChange }) {
  ...
  const handleDownload6MonthData = async () => {
    try {
      console.log('Downloading 6 month data from API...');
      
      // Use the new 6-month data API endpoint
      const apiUrl = `${SIX_MONTH_DATA_API_URL}?file=`;
      console.log('Fetching 6 month data from:', apiUrl);
      ...
      if (content.trim().startsWith('<') || content.includes('<html') || content.includes('<!DOCTYPE')) {
        throw new Error('Server returned HTML instead of JSON data. The API endpoint may be incorrect or returning an error page.');
      }
      ...
      try {
        ...
        throw new Error('Failed to parse response as JSON. Server may not be returning valid JSON.');
      }

      if (data && data.download_url) {
        window.open(data.download_url, '_blank');
        console.log('Initiated download for:', data.download_url);
      } else {
        throw new Error('Download URL not found in the API response.');
      }
      
      console.log('6 month data downloaded successfully');
    } catch (error) {
      console.error('Error downloading 6 month data:', error);
      alert(`Failed to download 6 month data: ${error.message}`);
    }
  };
