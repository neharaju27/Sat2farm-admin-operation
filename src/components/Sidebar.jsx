...
export default function Sidebar({ onLogout, user, onPageChange, currentPage }) {
  ...
  // Pages allowed for sales users
  const allowedPagesForSales = ['unlock-farm'];
  
  const handleNavigationClick = (page) => {
    if (isSalesUser && !allowedPagesForSales.includes(page)) {
      // Show restricted access message by navigating to the page
      // The AccessControl component will handle showing the restriction message
      onPageChange(page);
    } else {
      onPageChange(page);
    }
  ...
  return (
    <div className="h-full flex flex-col shadow-xl" style={{backgroundColor: '#0f2010'}}>
      ...
      <div className="sb-nav">
        ...
        <div 
          className={`sb-item ${currentPage === 'monthly-acreages' ? 'active' : ''} ${isSalesUser ? 'restricted' : ''}`}
        ...
        <div 
          className={`sb-item ${currentPage === 'farm-management' ? 'active' : ''} ${isSalesUser ? 'restricted' : ''}`}
          onClick={() => handleNavigationClick('farm-management')}
          style={{
            opacity: isSalesUser ? 0.5 : 1,
            cursor: isSalesUser ? 'not-allowed' : 'pointer',
            position: 'relative'
        ...
        <div 
          className={`sb-item ${currentPage === 'register' ? 'active' : ''} ${isSalesUser ? 'restricted' : ''}`}
        ...
        <div 
          ...
          onClick={() => handleNavigationClick('sales-clients')}
          style={{
            opacity: isSalesUser ? 0.5 : 1,
            cursor: isSalesUser ? 'not-allowed' : 'pointer',
            position: 'relative'
        ...
        <div 
          className={`sb-item ${currentPage === 'client-team' ? 'active' : ''} ${isSalesUser ? 'restricted' : ''}`}
          onClick={() => handleNavigationClick('client-team')}
          style={{
            opacity: isSalesUser ? 0.5 : 1,
            cursor: isSalesUser ? 'not-allowed' : 'pointer',
            position: 'relative'
        ...
        <div 
          className={`sb-item ${currentPage === 'client-alloc' ? 'active' : ''} ${isSalesUser ? 'restricted' : ''}`}
          onClick={() => handleNavigationClick('client-alloc')}
          style={{
            opacity: isSalesUser ? 0.5 : 1,
            cursor: isSalesUser ? 'not-allowed' : 'pointer',
            position: 'relative'
