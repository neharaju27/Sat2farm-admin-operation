...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // AuthContext's login function handles storing user data
      // App.jsx will detect the user change and redirect based on role
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-auto md:mr-auto md:ml-8">
        {/* Logo */}
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            ...
            <div className="space-y-3">
              <label className="flex items-center text-sm font-bold text-white">
                <User className="w-5 h-5 mr-3" style={{ color: 'rgb(252, 253, 254)' }} />
                Username
              </label>
            ...
            <div className="space-y-3">
              <label className="flex items-center text-sm font-bold text-white">
                <Lock className="w-5 h-5 mr-3" style={{ color: 'rgb(252, 253, 254)' }} />
                Password
              </label>
              <div className="relative group">
                <input
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-white text-black py-4 px-6 rounded-2xl font-bold text-lg shadow-2xl transform transition-all duration-300 hover:scale-105 hover:shadow-3xl hover:bg-gray-100 flex items-center justify-center group relative overflow-hidden disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              <span className="relative z-10 flex items-center">
                {isLoading ? 'Signing in...' : 'Sign In'}
                {!isLoading && <ArrowRight className="w-5 h-5 ml-3 transform group-hover:translate-x-2 transition-transform duration-300" />}
              </span>
